...
public class ObjectMatchingGameVestimentas: MonoBehaviour
{
    ...
    private void Update()
    {
        if (LifeCounterVestimentas.instance.GetLifesRemaining() > 0)
        {
            ...
            if (Input.GetMouseButtonUp(0))
            {
                ...
                if (hit.collider != null && hit.collider.TryGetComponent(out objectMatchForm) && matchId == objectMatchForm.Get_ID())
                {
                    ...
                    LifeCounterVestimentas.instance.Match();
                    this.enabled = false;

                    if (LifeCounterVestimentas.instance.CheckWin())
                    {
                        Win();
                    }
                    else
    ...
    private IEnumerator HideCorrectWrongIcon(String name)
    {
        ...
        {
            LifeCounterVestimentas.instance.LostLife();
            int lifes = LifeCounterVestimentas.instance.GetLifesRemaining();
            if (lifes <= 0)
            {
                lostScreen.SetActive(true);
                Lost();
